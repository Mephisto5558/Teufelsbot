  ...
  run: function (lang, client) {
    const
      ...
      getData = () => {
        switch (option) {
          case 'role-add':
          case 'role-remove': return input?.role ? input.role.id : (this.customReply(lang('needsRole')), null);
          case 'message': return input?.value ? input.value?.toString() : (this.customReply(lang('needsMessage')), null);
          case 'command': return ['owner-only', undefined].includes((client.prefixCommands.get(input?.value.split(' ')[0]) || client.slashCommands.get(input?.value.split(' ')[0]))?.category.toLowerCase()) ? (this.customReply(lang('commandNotFound')), null) : input.value?.toString();
        }
      };
