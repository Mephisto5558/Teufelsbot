...
module.exports = async function messageDelete() {
  ...
  if (this.user) embed.data.fields.push({ name: lang('messageDelete.author'), value: `${this.user.displayName} (\`${this.user.id}\`)`, inline: false });
  if (executor) embed.data.fields.push({ name: lang('executor'), value: `${executor.displayName} (\`${executor.id}\`)`, inline: false });
