...
function getMessageFromReference(client: Client, reference?: MessageReference): Message<true> | undefined {
  if (!reference?.guildId || !reference.messageId) return;
...
export default new Command({
  ...
  async run(lang) {
    ...
    try {
      if (msgId) SnowflakeUtil.deconstruct(msgId);
    }
    catch { msgId = undefined; }

    const msg = msgId
      ? await this.channel!.messages.fetch(msgId as Snowflake).catch(() => { /* empty */ })
    ...
    return this.channel!.send({
      content: lang('words', bold(match ? String(match.length + 1) : '0')),
