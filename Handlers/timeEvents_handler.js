...
module.exports = async client => {
  ...
  for (const file of readdirSync('./TimeEvents').filter(file => file.endsWith('.js'))) {
    const job = require(`../TimeEvents/${file}`);

    new CronJob(job.time, job.onTick, job.onComplete, true, job.timeZone, job.context, job.startNow, job.utcOffset);
